document.addEventListener("DOMContentLoaded", function () {
  ...
  const importNumberKeys = new Set([
    ...Object.keys(defaultSettings).filter(
      (key) => typeof defaultSettings[key] === "number",
    ),
    "ft_timer_end",
    "reviewLaterTime",
  ]);
  ...
  function sanitizeImportData(raw) {
    ...
    if ("platformSettings" in raw) {
      if (
        ...
        Array.isArray(raw.platformSettings)
      ) {
  ...
  function loadSettings() {
    chrome.storage.local.get(defaultSettings, function (items) {
      ...
      setToggle(
        "showBreakButton",
        items.autoStartBreaks !== false ? false : items.showBreakButton,
      );
  ...
  function updateBreakButtonVisibility(autoStartOn, shouldSave = false) {
    const breakButtonRow = document
      .getElementById("showBreakButton")
      ?.closest(".setting-row");
    if (breakButtonRow) {
  ...
  function disableToggles(disabled) {
    ...
    document
      .querySelectorAll(
        "input, select, button:not(.mode-btn):not(.platform-btn)",
      )
      .forEach((el) => {
    ...
    modeContainer.querySelectorAll(".mode-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const selectedMode = availableModes.find(
          (m) => m.id === btn.dataset.mode,
        );
        btn.querySelector(".mode-radio").style.borderColor = selectedMode
          ? selectedMode.color
          : "";
