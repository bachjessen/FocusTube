...
const FocusState = {
  get isWork() {
    return (
      ...
      CONFIG.timer.end &&
      CONFIG.timer.end > Date.now() &&
      CONFIG.timer.type === "work"
    );
  },
  get isBreak() {
    return (
...
const Utils = {
  ...
  clearInjectedStyles: function () {
    document
      .querySelectorAll(
        'style[data-ft-style], style[id^="ft-style"], style#ft-style',
      )
  ...
  applyVisualHidingClasses: function () {
    ...
    document.body.classList.toggle(
      "ft-hide-ig-stories",
      active && allowIg && CONFIG.visualHiding.igStories,
    );
    document.body.classList.toggle(
      "ft-hide-fb-stories",
      active && allowFb && CONFIG.visualHiding.fbStories,
    );
    document.body.classList.toggle(
      "ft-hide-yt-shorts-nav",
      active && allowYt && CONFIG.visualHiding.ytShortsNav,
    );
    document.body.classList.toggle(
      "ft-hide-yt-shorts-shelves",
      active && allowYt && CONFIG.visualHiding.ytShortsShelves,
    );
    document.body.classList.toggle(
  ...
  unlockVideo: function (options = {}) {
    ...
    if (this._mediaElements.size) {
      this._mediaElements.forEach((el) => {
        ...
        if (!state) return;
        el.muted = options.unmute ? false : state.muted;
        if (typeof state.volume === "number") {
          el.volume = options.unmute && state.volume === 0 ? 1 : state.volume;
        } else if (options.unmute && el.volume === 0) {
          el.volume = 1;
        }
...
const UI = {
  ...
  startPersistence: function (type, platform, onAllow, onBack, options = {}) {
    ...
    this.persistenceObserver = new MutationObserver((mutations) => {
      ...
      for (const mutation of mutations) {
        if (mutation.removedNodes.length) {
          for (const node of mutation.removedNodes) {
            if (
              node.id === this.overlayId ||
              (node.querySelector && node.querySelector(`#${this.overlayId}`))
            ) {
              removed = true;
              break;
            }
      ...
      if (removed || !document.getElementById(this.overlayId)) {
        if (Utils.isExtensionEnabled()) {
          this.create(type, platform, onAllow, onBack, options);
        }
      }
    });
  ...
  remove: function (options = {}) {
    ...
    Utils.unlockVideo({
      unmute: options.unmuteMedia === true,
      forcePlay: options.forcePlayMedia === true,
    });
    Utils.clearStatKeys();
  },
  ...
  showToast: function (title, msg) {
    if (!document.body) return;
    const exist = document.getElementById("ft-toast");
...
(function () {
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    if (changes.focusMode) {
      CONFIG.isFocusMode = changes.focusMode.newValue;
      Utils.toggleFocusClass(CONFIG.isFocusMode);
      Utils.applyVisualHidingClasses();
    }
    if (changes.platformSettings) {
      ...
      chrome.storage.local.get(
        ...
        (res) => {
          ...
          if (Site.isIG() && CONFIG.platformSettings.ig === "strict") {
            CONFIG.session.allowUntil = 0;
            CONFIG.session.platform = null;
            CONFIG.session.scope = null;
          }
          if (Site.isTT() && CONFIG.platformSettings.tt === "strict") {
            CONFIG.session.allowUntil = 0;
            CONFIG.session.platform = null;
            CONFIG.session.scope = null;
          }
    ...
    if (changes.restrictHiddenPlatforms)
      CONFIG.restrictHidden =
        changes.restrictHiddenPlatforms.newValue !== false;
    if (changes.visualHideHiddenPlatforms)
      CONFIG.visualHideHidden =
        changes.visualHideHiddenPlatforms.newValue !== false;
    if (changes.popup_visible_yt)
      CONFIG.popupVisibility.yt = changes.popup_visible_yt.newValue !== false;
    if (changes.popup_visible_ig)
      CONFIG.popupVisibility.ig = changes.popup_visible_ig.newValue !== false;
    if (changes.popup_visible_tt)
      CONFIG.popupVisibility.tt = changes.popup_visible_tt.newValue !== false;
    if (changes.popup_visible_fb)
    ...
    if (
      ...
      if (CONFIG.restrictHidden === false) {
        if (CONFIG.popupVisibility.yt === false && CONFIG.platformSettings)
          CONFIG.platformSettings.yt = "allow";
        if (CONFIG.popupVisibility.ig === false && CONFIG.platformSettings)
          CONFIG.platformSettings.ig = "allow";
        if (CONFIG.popupVisibility.tt === false && CONFIG.platformSettings)
          CONFIG.platformSettings.tt = "allow";
        if (CONFIG.popupVisibility.fb === false && CONFIG.platformSettings)
          CONFIG.platformSettings.fb = "allow";
    ...
    if (changes.hide_ig_stories) {
      CONFIG.visualHiding.igStories =
        changes.hide_ig_stories.newValue !== false;
      Utils.applyVisualHidingClasses();
    }
    if (changes.hide_fb_stories) {
      CONFIG.visualHiding.fbStories =
        changes.hide_fb_stories.newValue !== false;
      Utils.applyVisualHidingClasses();
    }
    ...
    document.dispatchEvent(
      new CustomEvent("ft-settings-changed", { detail: Object.keys(changes) }),
    );
  });
