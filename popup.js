  ...
  chrome.storage.local.get(
    ...
    (result) => {
      if (breakBtn) {
        ...
        const showBtn =
          workSessionEnded && !autoStartOn && result.showBreakButton !== false;
        setBreakPromptVisible(showBtn);
      }
      ...
      chrome.storage.onChanged.addListener((changes, area) => {
        if (area === "local") {
          ...
          if (
            ...
            changes.ft_work_session_ended.newValue === true
          ) {
            chrome.storage.local.get(
              ["autoStartBreaks", "showBreakButton"],
              (res) => {
          ...
          } else if (changes.ft_work_session_ended) {
            setBreakPromptVisible(false);
          }
          if (changes.darkMode) {
...
function setBreakPromptVisible(isVisible) {
...
function updatePlatformVisibility(visibility) {
  ...
  visiblePlatforms.forEach((platform, index) => {
    ...
    if (
      index === 0 ||
      itemsInRow >= targetPerRow ||
      (inLastRow && rows.length * targetPerRow === index)
    ) {
    ...
    if (startHeight !== endHeight) {
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          ...
          popupControls.addEventListener("transitionend", onEnd, {
            once: true,
          });
        });
      });
        ...
        chrome.runtime.sendMessage(
          ...
          (response) => {
            if (response && response.end)
              startTimerDisplay(response.end, "work");
          },
        );
      });
    }
...
function handleDocumentClick(e) {
  ...
  if (
    ...
    const icon = document.querySelector(
      `.platform-icon[data-platform="${currentPlatform}"]`,
    );
  ...
  if (reviewLater) {
    reviewLater.addEventListener("click", () => {
      chrome.storage.local.get(["ft_stats_blocked"], (res) => {
        ...
        chrome.storage.local.set({ reviewNextBlock: next }, () =>
          hideReviewPrompt(),
        );
      });
    });
